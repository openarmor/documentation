...
Audit generates numerous events and it is hard to distinguish if those events correspond to a *write access*, *read access*, *execute access*, *attribute change*, or *system call rule*, using Armor decoders and rules. This is why we use the *key* argument in audit rules to facilitate the processing of events by Armor. As previously explained, each audit rule has the option to add a descriptive *key* value to identify what rule generated a particular audit log entry. We will use a CDB list to determine the types of audit rule that has fire. This list will have the following syntax: ::
...
Editing ossec.conf
~~~~~~~~~~~~~~~~~~
Armor must be aware of the events detected by Audit. So, it is needs to be configured to read the audit log file: ::
...
Restarting OSSEC
~~~~~~~~~~~~~~~~~
Finally, we must restart Armor agent in order to apply the changes: ::
...
In this example, we are going to monitor every kind of access under the */home* directory: ::
    ...
    auditctl -w /home -p w -k audit-armor-w
    auditctl -w /home -p a -k audit-armor-a
    auditctl -w /home -p r -k audit-armor-r
    auditctl -w /home -p x -k audit-armor-x
...
Now we start getting alerts on account of the new audit rules::
  ...
  ** Alert 1487891035.24299: - audit,audit_configuration,
  ...
  type=CONFIG_CHANGE msg=audit(1487891033.538:2936): auid=1000 ses=346 subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 op="add_rule" key="audit-armor-w" list=4 res=1
  ...
  ** Alert 1487891043.24730: - audit,audit_configuration,
  ...
  Rule: 80705 (level 3) -> 'Auditd: Configuration changed'
  type=CONFIG_CHANGE msg=audit(1487891041.427:2937): auid=1000 ses=346 subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 op="add_rule" key="audit-armor-a" list=4 res=1
  audit.type: CONFIG_CHANGE
  audit.id: 2937
  audit.key: audit
  audit.list: 4
  ...
  ** Alert 1487891047.25161: - audit,audit_configuration,
  ...
  Rule: 80705 (level 3) -> 'Auditd: Configuration changed'
  type=CONFIG_CHANGE msg=audit(1487891045.481:2938): auid=1000 ses=346 subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 op="add_rule" key="audit-armor-r" list=4 res=1
  audit.type: CONFIG_CHANGE
  audit.id: 2938
  audit.key: audit
  audit.list: 4
  ...
  ** Alert 1487891049.25592: - audit,audit_configuration,
  ...
  Rule: 80705 (level 3) -> 'Auditd: Configuration changed'
  type=CONFIG_CHANGE msg=audit(1487891049.144:2939): auid=1000 ses=346 subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 op="add_rule" key="audit-armor-x" list=4 res=1
  audit.type: CONFIG_CHANGE
  audit.id: 2939
  audit.key: audit
  audit.list: 4
...
New File
  ...
  Alert::
    ...
    ** Alert 1487891161.28457: - audit,audit_watch_write,audit_watch_create,
    ...
    subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 key="audit-armor-w" type=CWD
...
Write Access
  ...
  Alert::
    ...
    ** Alert 1487891353.48010: - audit,audit_watch_write,
    ...
    subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 key="audit-armor-w"
...
Change Permissions
  ...
  Alert::
    ...
    ** Alert 1487891409.49498: - audit,audit_watch_attribute,
    ...
    subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 key="audit-armor-a" type=CWD
...
Read access
  ...
  Alert::
    ...
    ** Alert 1487891459.53222: - audit,audit_watch_read,
    ...
    subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 key="audit-armor-r" type=CWD
...
Here we choose to audit all commands run by a user who has admin privileges. The audit configuration for this is quite simple: ::
    ...
    $ auditctl -a exit,always -F euid=0 -F arch=b64 -S execve -k audit-armor-c
    $ auditctl -a exit,always -F euid=0 -F arch=b32 -S execve -k audit-armor-c
...
If the root user executes nano, the alert will look like this::
  ...
  ** Alert 1487892032.56406: - audit,audit_command,
  ...
  subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 key="audit-armor-c" type=EXECVE
...
By default, Armor is able to detect privilege escalation by analyzing the corresponding log in */var/log/auth.log*. The below example shows the homer user executing a root action: ::

    $ homer@springfield:/$ sudo ls /var/ossec/etc

Armor detects the action, extracting the *srcuser*, *dstuser* and *command* among other fields: ::
