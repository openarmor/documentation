...
This is for use only with the ``eventchannel`` log format.  By default, when Armor starts, it will read all log content from a given Windows Event Channel since Armor was last stopped.
Set this option to **yes** to override this behavior if desired.  Then Armor would only receive events that occur after the Armor agent is started.
...
schema in order to filter the events that Armor will process.
