...
1. Download the Armor Elasticsearch template::
    ...
    curl https://raw.githubusercontent.com/armor/armor/2.1/extensions/elasticsearch/armor-elastic5-template.json -o w-elastic-template.json
...
3. Load the template::
    ...
    curl -XPUT 'http://localhost:9200/_template/armor' -H 'Content-Type: application/json' -d @w-elastic-template.json
...
In a cluster with 1 node, the number of replicas should be 0::
    ...
    curl -XPUT 'localhost:9200/armor-*/_settings?pretty' -H 'Content-Type: application/json' -d'
    {
        "settings": {
            "number_of_replicas" : 0
        }
