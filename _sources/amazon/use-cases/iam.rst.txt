...
Following are some use cases for Armor rules built in for IAM events.


Create user account
-------------------

When we create a new user account in IAM, an AWS event is generated. As per the diagram at the beginning of this section, the log event flows to the Armor agent which passes it along to the Armor manager. The latter then analyzes the event and finds that it matches rule 80861. This results in an alert being generated, which can be seen in Kibana.
