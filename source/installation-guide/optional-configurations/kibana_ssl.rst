...
By default, the communications between Kibana (including the Armor app) and the web browser on end-user systems are not encrypted. It’s strongly recommended to configure Kibana to use SSL encryption and to enable authentication, next we briefly describe how to do this with a NGINX setup.
...
2. Generate the ``.htpasswd`` file. Replace ``armor`` with your chosen username (it must match with `auth_basic_user_file`):

  .. code-block:: bash

    $ htpasswd -c /etc/nginx/conf.d/kibana.htpasswd armor
