...
If you are running Armor server and Elastic Stack on separate systems & servers (distributed architecture), then it is important to configure SSL encryption between Filebeat and Logstash. This not applies to single-server architectures.
...
1. Edit file ``/etc/logstash/conf.d/01-armor.conf`` and uncomment the lines related to SSL under ``input/beats``. The active input section should now look like this:
...
1. On the **machine with Filebeat installed** (Armor server), fetch the Logstash server's SSL certificate file at ``/etc/logstash/logstash.crt`` and copy it into ``/etc/filebeat/logstash.crt``.

	Here is an **example** you might use to copy the SSL certificate from the Logstash server to Armor server where Filebeat is installed:
