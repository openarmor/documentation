...
(function ($) {
  if (window.matchMedia("(hover: hover)").matches) {
    ...
    $(".navbar-nav .dropdown-toggle").on("mouseleave", function (e) {
      ...
      if (
        $(this).closest(".dropdown").hasClass("show") &&
...
$(".desktop-search-form .header-search .btn-close").on("click", function (e) {
  /* Search bar close button clicked */ $(
    ".desktop-search-form .header-search .btn-close"
  ).addClass("d-none");
...
const useApiRedoc =
  compareVersion(DOCUMENTATION_OPTIONS.VERSION, minVersionRedoc) >= 0;
...
let oursHost = ["documentation.theopenarmor.org"];
