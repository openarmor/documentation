jQuery(function ($) {
  /*
   * Armor documentation - Version selector script
   * Copyright (C) 2021 Armor, Inc.
   *
   * EXECUTION FLOW
   * ==============
   *
  ...
  function checkCurrentVersion() {
    ...
    if (listOfVersions.length > 0) {
      selectVersionCurrent.html(
        "Version " +
          thisVersion +
          (thisVersion == listOfVersions[0] ? " (current)" : "")
      );
    } else {
  ...
  function addVersions() {
    ...
    const fullUrl =
      ...
      document.URL; /* Use document.URL as fix for Firefox */
    ...
    page = hasHttpProtocol
      ? fullUrl.split(document.location.host)[1]
      : fullUrl.split("/html")[1];
    ...
    for (let i = 0; i < versionsCopy.length; i++) {
      ...
      if (removedUrls[versionsCopy[i]]) {
        removedUrls[versionsCopy[i]] = removedUrls[versionsCopy[i]].map(
          function (url) {
            return normalizeUrl(url);
          }
        );
    ...
    const redirHistory = getRedirectionHistory(
      versionsCopy,
      thisVersion,
      page,
      newUrls,
    ...
    for (let i = 0; i < listOfVersions.length; i++) {
      ...
      if (!hasHttpProtocol && href.length > 0) {
        href = "https://documentation.theopenarmor.org" + href;
      }
      ele +=
        '<li><a href="' +
        href +
  ...
  function normalizeUrl(originalUrl) {
    if (!Array.isArray(originalUrl)) {
      ...
      if (
        normalizedURL.charAt(normalizedURL.length - 1) != "/" &&
   ...
   */
  function getRedirectionHistory(
    listOfVersions,
    verCurrent,
    page,
    newUrls,
    ...
    historyArray1 = fillUrls(
      "toBottom",
      historyArray,
      verCurrentNum,
      listOfVersions
    );
    historyArray2 = fillUrls(
      "toTop",
      historyArray,
      verCurrentNum,
      listOfVersions
    ...
    if (pageCreated !== false) {
      const pageCreatedNum = getReleaseNum(
        pageCreated["release"],
        listOfVersions
      );
      for (r in rtn) {
    ...
    const pageRemoved = getInfoRemovedUrl(
      rtn[listOfVersions[listOfVersions.length - 1]],
      removedUrls
    );
    if (pageRemoved !== false) {
      const pageRemovedNum = getReleaseNum(
        pageRemoved["release"],
        listOfVersions
      );
      for (r in rtn) {
          ...
          if (forVersions < verCurrentNum) {
            if (historyArray[listOfVersions[forVersions]] == null) {
              ...
              if (forVersions != listOfVersions.length - 1) {
                ...
                if (next != -1) {
                  historyArrayTemp[listOfVersions[forVersions]] =
                    historyArray[listOfVersions[next]];
                }
              }
            } else {
              historyArrayTemp[listOfVersions[forVersions]] =
                historyArray[listOfVersions[forVersions]];
            }
          }
        }
        if (direction == "toTop") {
          if (forVersions >= verCurrentNum) {
            if (historyArray[listOfVersions[forVersions]] == null) {
              ...
              if (forVersions != 0) {
                ...
                if (prev != -1) {
                  historyArrayTemp[listOfVersions[forVersions]] =
                    historyArrayTemp[listOfVersions[prev]];
                }
              }
            } else {
              historyArrayTemp[listOfVersions[forVersions]] =
