...
The Authd program has been improved in this version. Before Armor 2.1, the Armor API and the ``manage_agents`` tools could not register an agent while ``ossec-authd`` was running.
...
External libraries used by Armor have been updated to improve their integration with our components.
