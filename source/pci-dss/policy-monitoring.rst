...
Armor includes out-of-the-box, CIS baselines for Debian and Red Hat.  Other baselines could be created for other systems or applications as well, just by adding the corresponding rootcheck file:
...
In :ref:`Armor <installation>`, the rootcheck rules use this syntax in the rootcheck name: **{PCI_DSS: X.Y.Z}**, mapping all rootchecks to their relevant PCI DSS requirement.

Use cases
---------

In order to check SSH security settings and help meet requirement 2.2.4, we have developed the rootchecks ``system_audit_ssh``. In our example, when Armor runs a rootcheck scan, it is able to detect certain security deficiencies in the SSH configuration.
...
If enabled, the file ``archives.log`` stores every log parsed by the Armor engine, whether it becomes an alert or not:
...
.. thumbnail:: ../images/pci/policy_monitoring_2.png
    :title: Armor PCI DSS dashboard showing PCI DSS 2. alerts
    :align: center
    :width: 100%

.. thumbnail:: ../images/pci/p_monitoring_pci.png
    :title: Armor App PCI DSS section showing PCI DSS 2. alerts
    :align: center
    :width: 100%
