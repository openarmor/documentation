...
Armor already has a rule to monitor this::
...
You can configure Armor to monitor the Linux ``uptime`` command and alert when it is higher than a given threshold, like 2 in this example.
...
Armor can be configured to alert when a USB storage device is connected. This example is for a Windows agent.
