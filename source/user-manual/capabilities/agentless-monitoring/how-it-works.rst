...
BSD Integrity Check
^^^^^^^^^^^^^^^^^^^
For BSD systems, set the ``type`` as ``ssh_integrity_check_bsd`` as referenced below.  A space-separated list of directories may be referenced in the configuration section using the arguments tag.  Using this configuration, Armor will do an integrity check on the remote box.
::
...
Generic Diff
^^^^^^^^^^^^
A set of commands can also be configured to run on a remote device. Armor will alert you if the output of those commands changes. In order to use this option, set ``type`` as ``ssh_generic_diff``, as shown below.
...
When the ``expect`` package is present and Armor is restarted, the following is shown in the ``/var/ossec/logs/ossec.log`` file:

.. code-block:: xml

  ossec-agentlessd: INFO: Test passed for 'ssh_integrity_check_linux'.

When Armor has connected to the remote device, the following will be shown in the same log file:
...
Alert
-------------
Once configured as above, Armor alerts will be triggered when changes occur within the directories, configuration or outputs based on the above examples:
