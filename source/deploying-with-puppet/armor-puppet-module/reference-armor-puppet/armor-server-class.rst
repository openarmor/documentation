...
$ossec_extra_rules_config
  Using it, after enabling the Armor ruleset (either manually or via the automated script), take a look at the changes made to the ossec.conf file.
  You will need to put these same changes into the "$ossec_extra_rules_config" array parameter when calling the armor::server class.
...
$syslog_output
  Allows a Armor manager to send the OSSEC alerts to one or more syslog servers
...
.. note::
  Consequently, if you add or remove any of the Armor rules later on, you'll need to ensure you add/remove the appropriate bits in the $ossec_extra_rules_config array parameter as well.

.. _ref_server_email_alert:

``function armor::email_alert``
-------------------------------
...
``function armor::command``
---------------------------

$command_name
  Human readable name for armor::activeresponse usage.
